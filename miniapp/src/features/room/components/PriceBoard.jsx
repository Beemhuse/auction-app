...
export function PriceBoard({ state, phase }) {
  ...
  const reserveMissed = phase === 'ended' && state.outcome === 'RESERVE_NOT_MET';

  return (
    <section className={cn('card price-board', standing && `price-board-${standing}`)} aria-live="polite">
      <p className="price-label">{hasBids ? (phase === 'ended' && !reserveMissed ? 'Winning bid' : 'Highest bid') : 'Opening bid'}</p>
      ...
      {standing === 'leading' && (
        <p className="standing standing-leading">
          <Trophy size={16} aria-hidden="true" />{phase !== 'ended' ? 'You are the highest bidder' : reserveMissed ? 'Highest bid, reserve not met' : 'You have the winning bid'}
        </p>
      )}
